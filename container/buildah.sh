...
podman --cgroup-manager=cgroupfs run --rm -v "${SCRATCHMNT}":/mnt:rw --systemd=false maven:${MAVEN_VERSION} mvn dependency:get -DrepoUrl=${REPO_URL} -Dartifact=nl.thehyve:hypercube-api-server:${CONNECTOR_VERSION}:jar -Dtransitive=false -Ddest=/mnt/opt/connector/hypercube-api-server.jar
buildah config --env JAVACACERTDIR=${JAVACACERTDIR}"${NEWCONTAINER}"
buildah run "${NEWCONTAINER}" chown connector /opt/connector/hypercube-api-server.jar ${JAVACACERTDIR}/cacerts ${JAVACACERTDIR}
...
buildah tag hypercube-api-server docker.io/thehyve/hypercube-api-server:${CONNECTOR_VERSION}
